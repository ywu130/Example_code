---
title: "comparing transcriptome results mapped from different reference genome"
...
Import read count data from two files, with one (9192_counts.txt) mapped into whole genome (hg19), and the other file (9192_mini_gene_id_name_counts.txt) mapped to total transcriptome reference .

## Read in data

```{r read in data mapped into transcriptome reference}
...
```{r read in data mapped into hg19}
...
Acceptable correlation was observed for the table mapped from different references. Proceed with mapping to total transcriptome reference, as it is more time efficient when mapping.
