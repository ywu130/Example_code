...
traceback <- function(x, y, t1,t2,t3, start) {
  # COMPLETE THIS FUNCTION
  m = nchar(x)
  n = nchar(y)
  seqmatch = c('','')
  if(start==1){
    seqmatch[1] = paste(substr(x,m,m),seqmatch[1],sep ='')
    seqmatch[2] = paste(substr(y,n,n),seqmatch[2],sep ='')
    m = m-1
    n = n-1
    while(!is.na(t1[m+1,n+1])){
      if(t1[m+1,n+1]=='g'){
        seqmatch[1] = paste(substr(x,m,m),seqmatch[1],sep='')
        seqmatch[2] = paste(substr(y,n,n),seqmatch[2],sep='')
        m = m-1
        n = n-1
      } else if(t1[m+1,n+1]=='u'){
        seqmatch[1] =  paste(substr(x,m,m),seqmatch[1],sep='')
        seqmatch[2] = paste('-',seqmatch[2],sep='')
        m = m-1
        
      } else if(t1[m+1,n+1]=='l'){
        seqmatch[1] = paste('-',seqmatch[1],sep='')
        seqmatch[2] = paste(substr(y,n,n),seqmatch[2],sep='')
        n = n-1
...
affine_sequence_alignment <- function(x, y, s, d, e) {
  ...
  if(m[mrows,mcols]==score){
    ...
    start = 2#c(mrows-1,mcols)
  } else if(i_y[mrows,mcols]==score){
    start = 3#c(mrows,mcols-1)
  }
  
  a <- traceback(x, y, t1,t2,t3, start)
